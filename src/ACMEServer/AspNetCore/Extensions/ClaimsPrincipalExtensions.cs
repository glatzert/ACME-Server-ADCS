...
{
    public static Claim? GetAccountIdClaim(this ClaimsPrincipal principal)
        => principal.Claims.FirstOrDefault(c => c.Type == AcmeClaimTypes.AccountId);

    public static AccountId GetAccountId(this ClaimsPrincipal principal)
    {
        var accountIdClaim = principal.GetAccountIdClaim();
